...
the distance between C12-C4, the energies S0 and S1 and 
...
for filename in os.listdir(folder):
    nacs = []
    if filename == "PES_Highest_sf_closed_pf_mecp_S1S0_bhhlyp.out":
...
"""
em_df.columns = ["#Steps","C1-C4", "S0", "S1", "NAC"]
#em_df = em_df.sort_values(by="#C1-C4")
em_df = em_df.drop_duplicates(subset=["NAC"])
em_df.to_csv("DATA_PY/sf_closed_mecp_s1s0", index = False, sep='\t')
print(em_df)
